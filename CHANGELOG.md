...
The suites build real `TaskImpl`s and run them to completion through the real engine — that is the
whole point of them — so 0.19.0's craft timer recorded every fixture scenario as a finished craft.
One `/rstweaks selftest` completes a dozen in under a second, and only 8 are kept, so **running it
evicted the real 10k and 100k numbers from `/rstweaks stats`**.

Caught the first time the two were run side by side:

```
craft finished: 64 x product in 0ms
craft finished: 40 x widget in 0ms
craft finished: 4 x chest in 1ms
```

### The fix

A depth counter rather than a flag, because suites nest: `SelfTests` guards a whole run and
`TaskEngineSelfTest` guards itself, so a gametest calling a suite directly is covered too.

**Deliberately not `synchronized`.** The body is an entire suite and takes seconds; holding the
class monitor across it would block `record()` from Refined Storage's autocrafting threads for the
duration. The counter is only ever touched from the thread running the suite, and a stale read costs
one unrecorded craft rather than anything that matters.

Verified by counting rather than by reading: the gametest run now records **zero** fixture crafts
where it previously logged a dozen. 34/34 still pass.

### Also from that session

`/rstweaks selftest` passed **392 checks in the real world**, all 12 categories.

| | 0.20.0 | 0.21.0 |
|---|---|---|
| 100k benchmark | 33.6s (2,978/s) | 34.3s (2,916/s) |

2% apart — inside run-to-run variance, and consistent with 0.21.0 having been test-only. The 10k read
4.8s against 3.3s, explained by other crafts running in the world at the time: **the benchmark is
only comparable when nothing else is crafting.**
