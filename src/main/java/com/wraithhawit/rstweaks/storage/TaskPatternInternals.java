...
/**
 * The fields a batched step needs, which live on the wrong class to shadow.
 *
 * <p>{@code pattern}, {@code ingredients} and {@code root} are declared on {@code AbstractTaskPattern}, and
...
public interface TaskPatternInternals {
    ...
    /**
     * Whether this pattern makes what was asked for. Refined Storage sends the root's outputs to
     * the network as they are made and keeps every other pattern's in the task.
     */
    boolean rstweaks$root();
